defmodule Bible.Info do
  ...
  def get_bible_info(version_module) do
    ...
    book_number_map =
      gen_book_number_map(metadata)
      |> Enum.reduce(%{}, fn map, acc -> Map.merge(acc, map) end)

    verse_count_map =
      gen_verse_count_map(metadata)
      |> Enum.reduce(%{}, fn map, acc -> Map.merge(acc, map) end)

    verse_count_map =
      add_starting_verse(1, book_number_map, verse_count_map, [], 1)
      |> Enum.reduce(%{}, fn map, acc -> Map.merge(acc, map) end)
  ...
  def get_chapter_count(info, book) do
    with metadata = info["Metadata"],
         true <- Map.has_key?(metadata, book) do
      <<
        _::binary-size(3),
  ...
  def get_verse_count(info, book, chapter) do
    with metadata = info["Metadata"],
         ...
         true <- chapter >= 1 && chapter <= chapters do
      pos = 4 + chapter - 1
  ...
  defp new_load_metadata(version_module) do
    ...
    metadata =
      version_module.get_version_data
      |> String.split("\n")
  ...
  defp get_book_num_entry(book_map) do
    ...
    <<_::binary-size(2), book_number::unsigned-integer-size(8), _::binary>> = book_binary
    %{book_number => book}
  end
  ...
  defp get_verses_in_book(book_map) do
    {book, book_binary} = book_map
    <<_header::binary-size(4), verse_list::binary>> = book_binary
  ...
  defp add_starting_verse(book_number, book_number_map, verse_count_map, list, acc) do
    ...
    list = [%{book => %{"Verse Count" => verses, "Start Verse" => acc}}] ++ list
  ...
  defp get_verse_index(info, book, chapter, verse) do
    ...
    chaps = chapter - 1
    <<_header::binary-size(4), verse_list::binary-size(chaps), _::binary>> = metadata[book]

    verse_start = verse_count_map[book]["Start Verse"]

    verses_in_prior_chapters =
      for(<<byte::8 <- verse_list>>, do: byte)
      |> Enum.sum()
